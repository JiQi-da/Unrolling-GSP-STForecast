...
from lib.graph_learning_module import GNNExtrapolation, GNN_graph_construction, FeatureExtractor, GraphLearningModule
from lib.admm_block import ADMMBlock
from lib.backup_modules import layer_norm_on_data, layer_recovery_on_data
...
class UnrollingModel(nn.Module):
    def __init__(self, num_blocks, device, 
        ...
        self.linear_extrapolation = GNNExtrapolation(graph_info['n_nodes'], t_in, T, graph_info['u_edges'], n_heads, device)
        self.GNN_graph = GNN_graph_construction(graph_info['n_nodes'], n_heads, graph_info['u_edges'], graph_info['u_dist'], device, graph_sigma)
            ...
            self.model_blocks.append(nn.ModuleDict(
                {
                    'feature_extractor': FeatureExtractor(
                        n_in=signal_channels + 1,
                        n_out=feature_channels,
                        n_nodes=graph_info['n_nodes'],
                        n_heads=n_heads,
                        u_edges=graph_info['u_edges'],
                        u_dist=graph_info['u_dist'].to(self.device),
                        device=device,
                        alpha=GNN_alpha,
                    ),
    ...
    def forward(self, y):
        ...
        output = self.linear_extrapolation(y, self.GNN_graph)
        ...
        for i in range(self.num_blocks):
            ...
            # learn features
            features = feature_extractor(output, self.T, self.GNN_graph) # in (batch, T, n_nodes, n_heads, n_out)
            u_ew, d_ew = graph_learn(features)
            admm_block.u_ew = u_ew
            admm_block.d_ew = d_ew

...
class ADMMBlock(nn.Module):
    ...
    def __init__(self, T, n_nodes, n_heads, n_channels, u_edges, device, k_hop,
                 ADMM_info = {
                 'ADMM_iters':50,
                 'CG_iters': 3,
                 'PGD_iters': 3,
        ...
        # compute dedges
        self.d_edges = k_hop_neighbors(n_nodes, u_edges, k_hop)# torch.cat([self.u_edges, torch.arange(0, n_nodes, dtype=torch.long).unsqueeze(1).repeat(1,2)], dim=0)
        # place holder
        self.u_ew = None # place holder
    ...
    def apply_op_Lu(self, x):
        ...
        '''
        edges = self.u_edges
        edge_weights = self.u_ew
        B, T, n_nodes, n_heads, n_channels = x.size()
        n_edges = edges.size(1)
        node_i, node_j = edges[:,0], edges[:,1] # in (T, edges)
        # T_stamps = T_stamps.type(torch.long)
        weighted_i = edge_weights.unsqueeze(-1) * x[:, :, node_i]
        #E weighted_ji = edge_weights.view(B, -1, n_heads)[:, :, :, None] * (x[:, T_stamps.view(-1), node_j.view(-1)] - x[:, T_stamps.view(-1), node_i.view(-1)])# in (T*n_edges, n_heads, None) * (B, T*edges, n_head, n_channels)
        holder = torch.zeros((B, T, n_nodes, n_nodes, n_heads, n_channels), device=self.device)
        holder[:, :, node_i, node_j] = weighted_i
        in_features = holder.sum(2)
        return x - in_features

    def apply_op_Ldr(self, x):
        edges = self.d_edges
        edge_weights = self.d_ew
        B, T, n_nodes, n_heads, n_channels = x.size()
        # n_edges = edges.size(1)
        node_i, node_j = edges[:,0], edges[:,1] # in (T, edges)
        weighted_i = edge_weights.unsqueeze(-1) * x[:, :-1, node_i]# .view((T-1)*n_edges, n_heads, None) * (B, (T-1)*edges, n_head, n_channels)
        holder = torch.zeros((B, T, n_nodes, n_nodes, n_heads, n_channels), device=self.device)
        holder[:, 1:, node_i, node_j] = weighted_i
        in_features = holder.sum(2)
        y = x - in_features
        y[:,0] = torch.zeros_like(y[:,0]) # x[0] = 0
        return y
    ...
    def apply_op_Ldr_T(self, x):
        ...
        weighted_j = edge_weights.unsqueeze(-1) * x[:, 1:, node_j]# .view(T, n_edges, n_heads, n_channels) # in (T, edges, n_head, n_channels)
        holder = torch.zeros((B, T, n_nodes, n_nodes, n_heads, n_channels), device=self.device)
        holder[:, :-1, node_i, node_j] = weighted_j
        out_features = holder.sum(3)
        y = x - out_features # x[T] = x[T]
        y[:,0] = - out_features[:,0] # -= x[0] # x[0] = - out_features
        return y # x[T] = x[T]
    ...
    def apply_op_cLdr(self, x):
        ...
        n_edges = edges.size(1)
        node_i, node_j = edges[:,0], edges[:,1] # in (T, edges)
        T_stamps = torch.arange(0,T-1,1, dtype=torch.long)[:, None] * torch.ones((n_edges,), dtype=torch.long)
        # forward: weighted sum of in-nodes
        weighted_i = edge_weights.unsqueeze(-1) * x[:, :-1, node_i.view(-1)]# .view(T, n_edges, n_heads, n_channels) # in (T, edges, n_head, n_channels)
        holder = torch.zeros((B, T, n_nodes, n_nodes, n_heads, n_channels), device=self.device)
        holder[:, 1:, node_i, node_j] = weighted_i
        in_features = holder.sum(2)
        y = x - in_features
        ...
        # backward: weighted sum of out-nodes
        weighted_j = edge_weights.unsqueeze(-1) * y[:, 1:, node_j]
        holder = torch.zeros((B, T, n_nodes, n_nodes, n_heads, n_channels), device=self.device)
        holder[:, :-1, node_i, node_j] = weighted_j
        out_features = holder.sum(3)
        y = y - out_features
